...
if qtype mpg123; then
  ...
  random-sound(){
    ...
    local soundFile="$(find "$audioToolsDir/files" -type f -name '*mp3' 2> /dev/null | shuf | head -n1)"
    if [ -n "$soundFile" ]; then
      mpg123 -q "$soundFile"
    else
      error "$(printf "No sound files found in ${Colour_FilePath}%s${Colour_Off}" "$audioToolsDir/files")"
    fi
  }
fi
